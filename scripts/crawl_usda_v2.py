...
def main(args):
    ...
    synomyms = {}
    for off_key in off_raw.keys():
        open_food_fact = off_raw[off_key]
        wikidata = wikidata_raw[off_key] if off_key in wikidata_raw else {}

        food_fixture = {
            "model": "foods.food",
            "pk": str(uuid4()),
            "fields": {
                "wiki_id": wikidata.get("title"),
                "openfoodfacts_key": off_key,
                "usda_nbd_ids": None,
            }
        }

        names_multilinguagal = {
            f"name_{lc}": open_food_fact[lc]["name"] for lc in SUPPORTED_LANGS if lc in open_food_fact
        }

        for lc, data in wikidata.get("labels", {}).items():
            if lc in SUPPORTED_LANGS and f"name_{lc}" not in names_multilinguagal:
                names_multilinguagal[f"name_{lc}"] = data["value"]

        # check if english key is not starting with alpha character -> prolly we dont care since its likea chemical compound
        # normally they are already filtered if we kicked out th european additive E&&& values
        if "name_en" in names_multilinguagal and not isValidStartCharacter(names_multilinguagal["name_en"][0]):
            continue

        # Here we filter out potential duplicate keys. we resolve that by adding the first synonym we find to the name
        # should be very few,..hence also printy print
        keys_to_delete = []
        for lc, name in names_multilinguagal.items():
            if name in duplicates[lc]:
                print("ohshit....", name)

                if (aliases := wikidata.get("aliases", {}).get(lc.split("_")[1])) is not None:
        ...
        if "description" in wikidata:
            description_multilinguagal = {
                f"description_{lc}": wikidata["description"][lc]["value"]
                for lc in SUPPORTED_LANGS
                if lc in wikidata["description"]
            }
            food_fixture["fields"].update(description_multilinguagal)

        usda_nbd_ids = []
        for entry in wikidata.get("usda", []):
        ...
        for lc, aliases in wikidata.get("aliases", {}).items():
    ...
    # # v: [ USDA numbers ..]
    # foods = []
    # for openfoodfact_key, in wikidata_raw.items():
