...
import 'widgets/translatable_field.dart';
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Column(
      children: [
        ...
        const Divider(),
        if (recipe.ownerComment != null && recipe.ownerComment != "") ...[
          Padding(
              padding: const EdgeInsets.only(left: 20, right: 20, bottom: 10),
              child: Text("${recipe.ownerComment}")),
          const Divider(),
        ],
        // TODO: Deactivated tags for now
        // if (recipe.tags.isNotEmpty) const Divider(),
        ...
        //           const ElementsVerticalSpace(),
        //         ] +
        //         recipe.tags.map((e) => TagWidget(text: e.text)).toList(),
        //   ),
        // const Divider(),
        ...
        if ((recipe.sourceName != null && recipe.sourceName != "") ||
            recipe.sourcePage != null ||
            recipe.sourceUrl != null && recipe.sourceUrl != "") ...[
          const Divider(),
          if (recipe.sourceName != null && recipe.sourceName != "")
            Text("Source: ${recipe.sourceName}"),
          if (recipe.sourcePage != null)
            Text("Source Page: ${recipe.sourcePage}"),
          if (recipe.sourceUrl != null && recipe.sourceUrl != "")
            Text("Source Url: ${recipe.sourceUrl}"),
        ]

        // const Divider(),
        // // Text("Date Update: ${recipe.dateCreated.}"),
...
class IngredientsColumn extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    servingsCtrl.text = recipe.servings.toString();

    void onServingsChanges(value) async {
      final oldServingsCount = recipe.servings;
      final bool success =
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          ...widget.group.instructions.asMap().entries.map((entry) {
            return Padding(
...
class InstructionColumn extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return ListView(
      ...
      // shrinkWrap: true,
      scrollDirection: Axis.vertical,
      children: recipe.instructionGroups
          .map<Padding>(
            (group) => Padding(
    ...
    return Row(mainAxisAlignment: MainAxisAlignment.center, children: [
      Flexible(
        child: Padding(
          padding: const EdgeInsets.only(left: 20),
          child: TextButton(
            child: Text(recipe!.title, style: const TextStyle(fontSize: 20)),
            onPressed: () async {
              await ref
                  .read(recipeDetailsControllerProvider(recipeId).notifier)
                  .reloadRecipe();
